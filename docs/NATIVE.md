...
2. **The back button** goes through `navBack()` — the same one step back that
   the `‹` in every sheet header takes — instead of quitting, and only leaves
   the app from the map. It keeps no sheet list of its own, because the one it
   used to keep went stale as soon as new sheets were added. The age gate
...
the eviction-and-restore path and the shop, which hides by opacity and was
once mistaken for open by a handler that asked for its computed display.
