...
function toPy(list,ind){
  ...
  for(const b of list){
    switch(b.t){
      ...
      // A Tower brick has no kind — it just goes on the tile ahead. Without this
      // every 3D program printed robot.build("undefined").
      case "build":P.push(ind+(b.opt?'robot.build("'+b.opt+'")':"robot.build()"));break;
      case "climb":P.push(ind+"robot.climb()");break;
