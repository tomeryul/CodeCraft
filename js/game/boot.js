...
/* TEMP viewport diagnostics — splash only, removed when the game starts.
   Delete once the iOS full-bleed layout is confirmed on-device. */
function vpDebug(){
  ...
  const probe=css=>{const d=document.createElement("div");
    d.style.cssText="position:absolute;left:0;top:0;width:1px;visibility:hidden;pointer-events:none;"+css;
  ...
  el.textContent=
    ...
    "\nlvh "+probe("height:100lvh")+"  sat "+probe("height:env(safe-area-inset-top,0px)")+"  sab "+probe("height:env(safe-area-inset-bottom,0px)")+
    "\ngame "+Math.round(canvas.getBoundingClientRect().height)+"  VH "+VH+"  dpr "+(window.devicePixelRatio||1);
}
